...
class Generator(object):
	...
	def __init__(self, sco):
		self.shallow = Shallow(sco)	#shallow_feature
		self.deep = Deep_net(sco)	#deep_feature
		self.merge = Merge_net(sco)
		self.scope_name = sco

	def transform(self, oe_img, ue_img, is_training):
		img = tf.concat([oe_img, ue_img], 3)
		shallow_feature = self.shallow.shallow_generate(img, is_training)
		deep_feature = self.deep.deep_generate(img, is_training)
...
class Shallow(object):
	...
	def _create_variables(self, shape, scope):
		...
		with tf.variable_scope(self.scope):
			with tf.variable_scope('shallow_net'):
				with tf.variable_scope(scope):
					kernel = tf.Variable(tf.truncated_normal(shape, stddev = WEIGHT_INIT_STDDEV), name = 'kernel')
					bias = tf.Variable(tf.zeros(shape[-1]), name = 'bias')
		return (kernel, bias)

	def shallow_generate(self, image, is_training):
		out = image
		shape1 = [3, 3, 6, 40]
		kernel1, bias1 = self._create_variables(shape1, scope='conv1')
		out1 = conv2d(out, kernel1, bias1, use_relu = True, sn = False, is_training = is_training, Scope = self.scope + '/shallow_net/conv1/b')
		...
		out2 = conv2d(out1, kernel2, bias2, use_relu = True, sn = True, is_training = is_training,
		              Scope = self.scope + '/shallow_net/conv2/b')
		...
		out3 = conv2d(tf.concat([out1, out2], axis=-1), kernel3, bias3, use_relu = True, sn = True, is_training = is_training,
		              Scope = self.scope + '/shallow_net/conv3/b')
		...
		out4 = conv2d(tf.concat([out1, out2, out3], axis=-1), kernel4, bias4, use_relu = True, sn = True, is_training = is_training,
		              Scope = self.scope + '/shallow_net/conv4/b')
		...
		out5 = conv2d(tf.concat([out1, out2, out3, out4], axis = -1), kernel5, bias5, use_relu = True, sn = True,
		              ...
		              Scope = self.scope + '/shallow_net/conv5/b')
...
class Deep_net(object):
	...
	def deep_generate(self, image, is_training):
		out = image
		# encoder

		shape = [3, 3, 6, 16]
		kernel, bias = self._create_variables(shape, scope = 'encoder/conv1')
		out = deep_conv2d(out, kernel, bias, use_relu = True, sn = False, is_training = is_training, Scope = self.scope + '/deep_net/encoder/conv1/b')
		...
		out = deep_conv2d(out, kernel, bias, use_relu = True, sn = True, is_training = is_training, Scope = self.scope + '/deep_net/encoder/conv2/b')

		out = up_sample(out, 2)

		shape = [3, 3, 32, 20]
		kernel, bias = self._create_variables(shape, scope = 'decoder/conv1')
		out = deep_conv2d(out, kernel, bias, use_relu = True, sn = True, is_training = is_training, Scope = self.scope + '/deep_net/decoder/conv1/b', strides = [1, 1, 1, 1])

		out = up_sample(out, 2)

		shape = [3, 3, 20, 16]
		kernel, bias = self._create_variables(shape, scope = 'decoder/conv2')
		out = deep_conv2d(out, kernel, bias, use_relu = True, sn = True, is_training = is_training, Scope = self.scope + '/deep_net/decoder/conv2/b', strides = [1, 1, 1, 1])
...
def deep_conv2d(x, kernel, bias, use_relu = True, Scope = None, BN = True, sn = True, is_training = False, strides=[1, 2, 2, 1]):
...
def deep_deconv2d(x, kernel, strides):
	with tf.device(device2):
